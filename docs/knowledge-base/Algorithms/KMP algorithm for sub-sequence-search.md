...
KMP avoids redundant comparisons by precomputing a `next` array, where `next[i]` is the **length of the longest proper prefix** of `pattern[0..i]` that is **also a suffix** — i.e., how much of the pattern beginning we've already effectively "re-matched" for free. When a mismatch occurs at pattern position `j`, instead of restarting from 0, we fall back to `j = next[j-1]`, realigning the pattern so that the already-matched border stays in place. No character of the target is ever re-examined, yielding O(n + m) time overall. 
...
![[Pasted image 20260629090315.png|445]]

So,  we need to **pre-compute** the `next` array to record the longest suffix  :   

| Array | A   | B   | C   | D   | A   | B   | C   |
| ----- | --- | --- | --- | --- | --- | --- | --- |
| next  | -1  | 0   | 0   | 0   | 1   | 2   | 3   |
when comparing failed at character $i$, then we should move the re-compare location to `next[i-1]`
